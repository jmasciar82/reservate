...
export default function CourtCard({ court, clubName, onEdit, onDelete }: CourtCardProps) {
  ...
  return (
    <div
      className={`group relative overflow-hidden flex flex-col justify-between min-h-64 rounded-xl border ${design.border} ${design.courtBg} transition-all duration-300 hover:shadow-lg hover:shadow-${design.accent}/10 hover:scale-[1.02]`}
    >
      {/* Patrón de fondo (Field) */}
      {design.renderField()}

      <div className="relative z-10 p-5 flex flex-col h-full bg-gradient-to-t from-zinc-950/90 via-zinc-950/60 to-transparent">
        <div>
          <div className="flex justify-between items-start gap-3 mb-2">
            <div className="flex flex-col gap-1 min-w-0">
              ...
              <span className={`text-xs font-bold uppercase tracking-wider ${design.accent} drop-shadow`}>
                {design.label}
              </span>
            </div>
            ...
            <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
              {court.isActive ? (
                <span className="flex items-center text-[10px] font-bold text-primary bg-primary/20 px-2 py-0.5 rounded-full border border-primary/30 backdrop-blur-sm">
                  <Check className="w-2.5 h-2.5 mr-1" /> Activa
                </span>
              ) : (
                <span className="flex items-center text-[10px] font-bold text-zinc-300 bg-zinc-800/80 px-2 py-0.5 rounded-full border border-zinc-600 backdrop-blur-sm">
                  Inactiva
                </span>
              )}

              {court.isCovered ? (
                <span className="flex items-center text-[10px] font-bold text-sky-300 bg-sky-500/20 px-2 py-0.5 rounded-full border border-sky-400/30 backdrop-blur-sm">
                  <Home className="w-2.5 h-2.5 mr-1" /> Techada
                </span>
              ) : (
                <span className="flex items-center text-[10px] font-bold text-amber-300 bg-amber-500/20 px-2 py-0.5 rounded-full border border-amber-500/30 backdrop-blur-sm">
        ...
        <div className="mt-auto pt-6 flex items-end justify-between">
          <div className="flex items-center gap-1.5 text-zinc-200 bg-zinc-950/60 backdrop-blur-md py-1.5 px-3 rounded-lg border border-zinc-800/50 w-fit">
          ...
          <div className="flex justify-end gap-2 transition-opacity duration-200">
            <button
              ...
              className="p-2 text-zinc-300 hover:text-white hover:bg-zinc-800/80 rounded-lg transition-all backdrop-blur-sm"
              title="Editar cancha"
            >
            ...
            <button
              ...
              className="p-2 text-zinc-300 hover:text-red-400 hover:bg-red-500/20 rounded-lg transition-all backdrop-blur-sm"
